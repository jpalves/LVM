 ...
 */
function filter_text_image_dfmtex($imagefile, $tex, $height, $width, $align, $alt) {
    global $CFG, $OUTPUT;
    ...
    // Build the output.
    $anchorcontents = "<img class=\"dfmtexrender\"  src=\"";
    if ($CFG->slasharguments) {        // Use this method if possible for better caching
        $anchorcontents .= "$CFG->wwwroot/filter/dfmtex/pix.php/$imagefile";
    } else {
        $anchorcontents .= "$CFG->wwwroot/filter/dfmtex/pix.php?file=$imagefile";
    }
    $anchorcontents .= "\" $style/>";
    /*
    if (!file_exists("$CFG->dataroot/filter/dfmtex/$imagefile") && has_capability('moodle/site:config', get_context_instance(CONTEXT_SYSTEM))) {
        $link = '/filter/dfmtex/texdebug.php';
        $action = null;
    } else {
        $link = new moodle_url('/filter/dfmtex/displaytex.php', array('texexp'=>$tex));
 ...
 * TeX filtering class.
 */
class filter_dfmtex extends moodle_text_filter {
    function filter($text, array $options = array()) {

        global $CFG, $DB;

        /// Do a quick check using stripos to avoid unnecessary work
		if (!preg_match('/<tex/i',$text) and !preg_match('/&lt;tex&gt/i',$text)) {
			return $text;
		} 
...
#    }
        ...
        preg_match_all("/&lt;tex&gt;(.+?)&lt;\/tex&gt;|<tex\s?\/?>(.+?)<\/tex\s?\/?>/is", $text, $matches);
        for ($i=0; $i<count($matches[0]); $i++) {
            ...
            $md5 = md5($texexp);
            if (!$DB->record_exists("cache_filters", array("filter"=>"dfmtex", "md5key"=>$md5))) {
                $texcache = new stdClass();
                $texcache->filter = 'dfmtex';
                $texcache->version = 1;
                $texcache->md5key = $md5;
            ...
            $filename = $md5 . ".{$CFG->filter_dfmtex_convertformat}";
			//echo $texexp;
            $text = str_replace( $matches[0][$i], filter_text_image_dfmtex($filename, $texexp, 0, 0, $align, $alt), $text);
        }
        return $text;
    }
}
