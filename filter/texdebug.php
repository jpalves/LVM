 ...
 */
    ...
    if ($action=='ShowDB' or $action=='DeleteDB') {
        $md5 = md5($texexp);
        $texcache = $DB->get_record("cache_filters", array("filter"=>"dfmtex", "md5key"=>$md5));
    }
    ...
    function TexOutput($expression, $graphic=false) {
        ...
        if (is_file($CFG->filter_tex_pathlatex)) {
            $output .= "latex executable ($CFG->filter_dfmtex_pathlatex) is readable<br />\n";
        }
        else {
            $executables_exist = false;
            $output .= "<b>Error:</b> latex executable ($CFG->filter_dfmtex_pathlatex) is not readable<br />\n";
        }
        if (is_file($CFG->filter_tex_pathdvips)) {
            $output .= "dvips executable ($CFG->filter_dfmtex_pathdvips) is readable<br />\n";
        }
        else {
            $executables_exist = false;
            $output .= "<b>Error:</b> dvips executable ($CFG->filter_dfmtex_pathdvips) is not readable<br />\n";
        }
        if (is_file($CFG->filter_tex_pathconvert)) {
            $output .= "convert executable ($CFG->filter_dfmtex_pathconvert) is readable<br />\n";
        }
        else {
            $executables_exist = false;
            $output .= "<b>Error:</b> convert executable ($CFG->filter_dfmtex_pathconvert) is not readable<br />\n";
        }
        ...
        $img = "$latex->temp_dir/$md5.{$CFG->filter_dfmtex_convertformat}";
        ...
        $cmd = "$CFG->filter_dfmtex_pathlatex --interaction=nonstopmode $tex";
        $output .= execute($cmd);

        // step 2: dvips command
        $cmd = "$CFG->filter_dfmtex_pathdvips -E $dvi -o $ps";
        $output .= execute($cmd);

        // step 3: convert command
        $cmd = "$CFG->filter_dfmtex_pathconvert -density 240 -trim $ps $img ";
        $output .= execute($cmd);

        if (!$graphic) {
            echo $output;
        } else if (file_exists($img)){
            send_file($img, "$md5.{$CFG->filter_dfmtex_convertformat}");
        } else {
    ...
    function slasharguments($texexp) {
        ...
        echo "<img src=\"$CFG->wwwroot/filter/dfmtex/pix.php/$image\" align=\"absmiddle\"></p>\n";
        echo "<p>Otherwise set it to file.php?file=/1/pic.jpg ";
        echo "It should display correctly as ";
        echo "<img src=\"$CFG->wwwroot/filter/dfmtex/pix.php?file=$image\" align=\"absmiddle\"></p>\n";
